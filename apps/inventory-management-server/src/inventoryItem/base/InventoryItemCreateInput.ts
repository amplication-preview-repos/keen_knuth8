/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DamageReportCreateNestedManyWithoutInventoryItemsInput } from "./DamageReportCreateNestedManyWithoutInventoryItemsInput";
import {
  ValidateNested,
  IsOptional,
  IsDate,
  IsString,
  IsInt,
} from "class-validator";
import { Type } from "class-transformer";
import { ExpiryReportCreateNestedManyWithoutInventoryItemsInput } from "./ExpiryReportCreateNestedManyWithoutInventoryItemsInput";

@InputType()
class InventoryItemCreateInput {
  @ApiProperty({
    required: false,
    type: () => DamageReportCreateNestedManyWithoutInventoryItemsInput,
  })
  @ValidateNested()
  @Type(() => DamageReportCreateNestedManyWithoutInventoryItemsInput)
  @IsOptional()
  @Field(() => DamageReportCreateNestedManyWithoutInventoryItemsInput, {
    nullable: true,
  })
  damageReports?: DamageReportCreateNestedManyWithoutInventoryItemsInput;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  expiryDate?: Date | null;

  @ApiProperty({
    required: false,
    type: () => ExpiryReportCreateNestedManyWithoutInventoryItemsInput,
  })
  @ValidateNested()
  @Type(() => ExpiryReportCreateNestedManyWithoutInventoryItemsInput)
  @IsOptional()
  @Field(() => ExpiryReportCreateNestedManyWithoutInventoryItemsInput, {
    nullable: true,
  })
  expiryReports?: ExpiryReportCreateNestedManyWithoutInventoryItemsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsInt()
  @IsOptional()
  @Field(() => Number, {
    nullable: true,
  })
  quantity?: number | null;
}

export { InventoryItemCreateInput as InventoryItemCreateInput };
